...
def split_images(img_path: str, split_ind: int, n_splits: int = 4, overlap: int = 150,
    ...
    files = glob.glob(f'{img_path}/{filename_pattern}')
    files.sort()
    ...
    for f in files[:max_num_files]:
        output_filename = f"{full_path}/{f.split('/')[-1]}"
        han = fits.open(f)
        for ii in range(1, 4):  # loop over the first 4 HDU extensions
            d = han[ii].data
        ...
        logging.debug(f"Writing split for {f} to {output_filename}")
...
def main():
    parser = argparse.ArgumentParser(description="Make copies of a images in diff_dir matching pattern by "
                                                 "splitting those images into n-splits files with a "
                                                 "overlap pixels in common.")
    parser.add_argument('diff_dir', help="Name of the directory holding the difference images to be split")
    parser.add_argument('split_dir',
                        help='Base name of the directory to distribute split images into. (%(default)s)')
    parser.add_argument('--n-splits',
                        default=4, help="How many chunks should the images be split into? (%(default)s)",
                        type=int)
    parser.add_argument('--split-index', default=None,
                        help="Only cutout out this split-index of n-splits chunks. (%(default)s)",
                        type=int)
    parser.add_argument('--overlap', default=150,
                        help="How many pixels of overlap between chunks? (5(default)s)",
                        type=int)
    parser.add_argument('--pattern', help="Pattern to match files that should be split. (%(default)s)",
                        default='DIFFEXP*.fits')
    ...
    logging.info(f"Splitting DIFFEXP*.fits files in {im_filepath} into {args.n_splits} chunks "
                 f"stored in {split_img_path}")
