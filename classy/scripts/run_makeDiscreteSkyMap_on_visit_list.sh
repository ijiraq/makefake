...
### Wes adjustment to use processCCD data because no plants exist for testing yet
ccdOutputs="processCcdOutputs"
...
visit_list="visitLists/${field}/${field}_visit_list.txt"
[ -f ${visit_list} ] || logmsg ERROR "Didnt find ${visit_list}" 1

# build input file for ${command}
echo "--rerun ${ccdOutputs}:${coAdd}" > ${filename} 
echo "--config skyMap.projection=TAN skyMap.patchInnerDimensions=6000,6000 skyMap.pixelScale=0.185" >> ${filename}
