C-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
      ...
      real(kind=8) gb, alpha, mag_limit, H_limit, MA, ran3
...
C     if in_sample=1 then we include this object, otherwise skip
     ...
     $     drad = Pi/180.0d0, mu = TwoPi**2)
      parameter (gb = 0.15, mag_limit=28, H_limit=14, version=4)
...
C     Loop over making fake objects until we have max_objects in frame
     ...
     $     (niter .lt. max_iters))
         ...
         a = 30 + ran3(seed)*220
         e = ran3(seed)*0.999
         inc = Pi*ran3(seed)/2.0
         node = TwoPi*ran3(seed)
         peri = TwoPi*ran3(seed)
         M = TwoPi*ran3(seed)
         ran_mag = mag_limit - (mag_limit - 22.0)*ran3(seed)
