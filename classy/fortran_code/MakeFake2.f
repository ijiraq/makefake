...
C     if in_sample=1 then we include this object, otherwise skip
     ...
     $     drad = Pi/180.0d0, mu = TwoPi**2)
      ...
      real(kind=8) ra, dec, delta, radius, mag, ddec, dra, ra_cen, dec_cen
...
C     empty of sources.
      ...
      CALL parse_args(ra_cen, dec_cen, fake_jday, max_objects)

C     set the date to start of 2022.. this is the base date for generating positions.
      obs_jday = 2459580.5
      epoch_M = obs_jday
      ...
      write(in_str, *) "PlantList",ra_cen,"_",dec_cen,"_",jd,".txt"
...
C     Move the Mean Annomally from jday to epoch_M
      ...
      subroutine parse_args(ra, dec, jd, max_objects)
